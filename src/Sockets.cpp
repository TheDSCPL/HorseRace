...
void Network::start_server()
{
  ...
  serv_addr.sin_port = htons(Utils::s2b(Properties::getDefault().getProperty("PORT")));
  ...
  cout << "Telnet server started on " << get_ip(sockfd) << ':' << PORT << ". Accepting clients." << endl << endl;
  clog( "Telnet server started on " << get_ip(sockfd) << ':' << PORT << ". Accepting clients." );
  server_running=true;
}

void Network::writeline(int so, string line,bool paragraph)
{ // Envia uma string para um socket
  if(so<0)
    {LOCK;cerr << line << endl;UNLOCK;}
  else
  {
...
bool Network::readline(int socketfd, string &line)
{ // Lê uma linha de um socket e retorna false se o socket se tiver fechado
...
void Network::broadcast (int origin, string text)
{ // Envia uma mensagem para todos os clientes ligados exceto 1
  ...
  if(clients[origin]==LOGGED_OFF)
    clog( "Socket " << origin << " (not logged in) broadcasted: \"" << text << "\"" )
  else
    clog( "Socket " << origin << " (user_id=" << clients[origin] << ") broadcasted: \"" << text << "\"" )
...
void Network::shutdown_server()
{
  ...
  clients.insert(PAIR(IN_SERVER,s_u_i));
...
void Network::cliente()
{
  ...
  clog( "New client accepted on socket " << client_socket << ". Thread where this socket is being heard: " << pthread_self() );
